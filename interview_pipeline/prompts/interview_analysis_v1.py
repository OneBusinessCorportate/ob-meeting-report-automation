...
═══════════════ ЧТО ОЦЕНИВАТЬ ═══════════════
- summary: содержательное резюме на РУССКОМ (3–6 предложений): кто кандидат, о
  чём говорили, как прошёл разговор.
  ВАЖНО: ПЕРВОЕ предложение — это законченный самостоятельный вывод (кто кандидат
  + ключевой итог по пригодности), потому что в коротком отчёте может
  показываться ТОЛЬКО первое предложение. Не начинай с воды вроде «На
  собеседовании обсуждались…»; сразу давай суть.
...
- theses: массив из РОВНО 5 объектов (по одному на каждый тезис, в порядке
  id 1→5): {"id", "title", "score" (0–10), "comment" — КОНКРЕТНОЕ обоснование
  оценки со ссылкой на факт/слова кандидата из звонка, без общих фраз}.
...
- reasoning: объяснение итогового решения на русском. ПЕРВОЕ предложение —
  само решение одной чёткой фразой (в коротком отчёте может показываться только
  оно), далее — обоснование по тезисам.
